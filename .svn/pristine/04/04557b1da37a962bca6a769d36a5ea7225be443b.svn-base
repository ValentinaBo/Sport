...
public class ViewsController {
	...
	public ModelAndView updateCliente(@ModelAttribute Cliente cliente, HttpServletRequest request) {
			Login login = new Login();
			Role role = new Role();
			login.setPassword(request.getParameter("psw"));
			login.setUsername(cliente.getUsername());
			role.setRoleId(0);
			role.setUsername(cliente.getUsername());
	...
	public ModelAndView creaBiglietto(@ModelAttribute Biglietto biglietto, HttpServletRequest request) {
		...
		Cliente cliente = clienteDAO.read(biglietto.getIdClienteBigliettoFK());
			
		return new ModelAndView("redirect:/areapersonale?username=" + cliente.getUsername());
	}
	...
	public ModelAndView areaPersonale (ModelAndView model, HttpServletRequest request, Principal principal) {
		...
		if (request.isUserInRole("ROLE_ADMIN")) {
			...
			/*ID - Se il gestore ha fatto l'accesso, la richiesta con l'ID vuol dire che sta guardando le aree personali 
			dei clienti, quindi l'area da mostrare può essere solo di un cliente e non del gestore.*/
			...
			//USERNAME (può essere solo il gestore)
		...
		//Login del cliente - solo con username
			String user = request.getParameter("username");
	...
	/*Handler per la visualizzazione dell'incasso */
	@RequestMapping(value="/showTotal", method = RequestMethod.GET)
	public ModelAndView showTotal (ModelAndView model, HttpServletRequest request) {
		String tab = request.getParameter("value");
		if (tab.equals("totali")) {
